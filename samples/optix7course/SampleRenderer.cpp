...
// #endif
      OWLBuffer vertexBuffer 
        = owlBufferCreateEXT(context,OWL_FLOAT3,mesh.vertex.size(),
                             mesh.vertex.data());
      OWLBuffer indexBuffer  
        = owlBufferCreateEXT(context,OWL_INT3,mesh.index.size(),
                             mesh.index.data());
      OWLBuffer normalBuffer 
        = mesh.normal.empty()
        ? nullptr
        : owlBufferCreateEXT(context,OWL_FLOAT3,mesh.normal.size(),
                             mesh.normal.data());
      OWLBuffer texcoordBuffer
        = mesh.texcoord.empty()
        ? nullptr
        : owlBufferCreateEXT(context,OWL_FLOAT2,mesh.texcoord.size(),
                             mesh.texcoord.data());
      // create the geom
      OWLGeom geom
  ...
  void SampleRenderer::resize(void *fbPointer, const vec2i &newSize)
  {
    ...
    this->fbSize = newSize;
    fbColor = owlBufferCreateEXT(context,OWL_FLOAT4,fbSize.x*fbSize.y,nullptr);
