...
{
  if (!accumBuffer)
    accumBuffer = owlBufferCreateEXT(context,OWL_FLOAT4,1,nullptr);
  ...
  : setup(stp)
{
  ...
  poolballs.centerBuffer
    = owlBufferCreateEXT(context,OWL_FLOAT3,poolballs.center.size(),&poolballs.center[0]);
  poolballs.rotationBuffer
    = owlBufferCreateEXT(context,OWL_USER_TYPE(linear3f),poolballs.rotationMatrices.size(),&poolballs.rotationMatrices[0]);
  poolballs.colorBuffer
    = owlBufferCreateEXT(context,OWL_FLOAT3,poolballs.color.size(),&poolballs.color[0]);
  poolballs.kd_mapBuffer
    = owlBufferCreateEXT(context,OWL_TEXTURE,poolballs.kd_maps.size(),&poolballs.kd_maps[0]);
  ...
  OWLBuffer lightBuffer
    = owlBufferCreateEXT(context,OWL_USER_TYPE(BasicLight),2,&lights);
